module SPI_iNEMO1(SS_n,SCLK,MISO,MOSI,INT);
  ...
  input logic SS_n;				// active low serf select
  input logic SCLK;				// Serial clock
  input logic MOSI;				// serial data in from monarch
  output logic MISO;				// serial data out to monarch
  ...
  function [7:0] response (input logic [7:0] in_byte);
